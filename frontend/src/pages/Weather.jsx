...
//   const [isSearching, setIsSearching] = useState(false);
//   const [locationLoading, setLocationLoading] = useState(true);
//   const [karnatakaCities, setKarnatakaCities] = useState([]);
//   const [showCityDropdown, setShowCityDropdown] = useState(false);

//   useEffect(() => {
//     getUserCurrentLocation();
//     fetchKarnatakaCities();
//   }, []);

//   const fetchKarnatakaCities = async () => {
//     try {
//       const response = await axios.get('/api/weather/cities');
//       if (response.data.success) {
//         setKarnatakaCities(response.data.cities);
//       }
//     } catch (error) {
...
//     if (!navigator.geolocation) {
//       toast.error("Geolocation is not supported by your browser");
//       setLocation({ lat: 14.4667, lng: 75.9167, city: "Davanagere" });
//       fetchWeatherData(14.4667, 75.9167);
//       setLocationLoading(false);
//       return;
//     }
...
//             city: cityName 
//           });
//           await fetchWeatherAnalytics(latitude, longitude);
//           toast.success(`Weather data for ${cityName} loaded!`);
//         } catch (error) {
//           console.error("Error getting city name:", error);
//           setLocation({ lat: latitude, lng: longitude });
//           await fetchWeatherAnalytics(latitude, longitude);
...
//         let errorMessage = "Unable to get your location. Using Davanagere as default.";
//         toast.error(errorMessage);
        
//         setLocation({ lat: 14.4667, lng: 75.9167, city: "Davanagere" });
//         await fetchWeatherAnalytics(14.4667, 75.9167);
//         setLocationLoading(false);
//       },
//       {
//         enableHighAccuracy: true,
...
//       "17.3297,76.8343": "Kalaburagi",
//       "13.9299,75.5681": "Shimoga",
//       "13.0067,76.0990": "Hassan",
//       "14.2250,76.3950": "Chitradurga",
//       "15.1500,76.9333": "Bellary",
//       "17.9229,77.5175": "Bidar",
//       "16.2100,77.3550": "Raichur",
//       "13.1333,78.1333": "Kolar",
//       "12.5242,76.8958": "Mandya",
//       "13.3389,74.7451": "Udupi"
//     };
...
//     Object.entries(karnatakaCities).forEach(([coords, city]) => {
//       const [knownLat, knownLng] = coords.split(',').map(Number);
//       const distance = Math.sqrt(
//         Math.pow(lat - knownLat, 2) + Math.pow(lng - knownLng, 2)
//       );
...
//   const fetchWeatherAnalytics = async (lat, lng, city = null) => {
//     try {
//       setLoading(true);
//       setError(null);
      
//       const params = city ? { city } : { lat, lng };
//       const response = await axios.get('/api/weather/analytics', { params });

//       if (response.data.success) {
//         setData(response.data.analytics.current);
//         setAnalytics(response.data.analytics);
//         toast.success("Real-time weather analytics loaded!");
//       } else {
...
//       toast.error("Failed to load real-time data. Using demo data.");
//       // Fallback to mock data
//       setData(getMockWeatherData(lat, lng));
//       setAnalytics(getMockAnalytics(lat, lng));
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     try {
//       setIsSearching(true);
//       toast.loading(`Fetching weather for ${cityInput}...`);
      
//       await fetchWeatherAnalytics(null, null, cityInput);
//       setCityInput("");
//       setShowCityDropdown(false);
//       toast.success(`Weather data for ${cityInput} loaded!`);
//     } catch (error) {
//       console.error("City search error:", error);
...
//     setShowCityDropdown(false);
//     fetchWeatherAnalytics(city.coordinates.lat, city.coordinates.lng);
//   };
...
//     const locationKey = `${lat.toFixed(4)},${lng.toFixed(4)}`;
//     const locationInfo = cities[locationKey] || { city: "Karnataka", country: "India" };
...
//   const getMockAnalytics = (lat, lng) => {
//     return {
//       statistics: {
//         avgTemperature: Math.round(25 + Math.random() * 10),
//         maxTemperature: Math.round(30 + Math.random() * 10),
//         minTemperature: Math.round(20 + Math.random() * 5),
//         totalRainfall: Math.round(Math.random() * 20),
...
//           />
//           <p className="text-green-700 text-lg mb-2">Detecting your location...</p>
//           <p className="text-green-600 text-sm">Fetching real-time weather data</p>
//         </motion.div>
//       </div>
//     );
//   }
...
//                   placeholder="Search Karnataka cities (e.g., Bengaluru, Mysuru, Hubli)"
//                   className="w-full p-3 border border-red-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm bg-white text-gray-900"
//                   disabled={isSearching}
//                 />
...
//                         >
//                           <div className="font-medium text-green-800">{city.name}</div>
//                           <div className="text-sm text-green-600">Karnataka, India</div>
//                         </motion.div>
//                       ))}
//                   </div>
//                 )}
...
//           <p className="text-green-600 text-lg">
//             Real-time weather analytics for smart irrigation in Karnataka
//           </p>
//         </motion.div>
...
//               className="bg-white rounded-2xl shadow-lg p-6"
//             >
//               <h2 className="text-2xl font-bold text-blue-700 mb-4">Weather Analytics</h2>
              
//               <div className="space-y-4">
//                 <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
//                   <span className="text-gray-600">Average Temperature</span>
//                   <span className="font-semibold text-blue-600">{analytics.statistics.avgTemperature}°C</span>
//                 </div>
                
//                 <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
//                   <span className="text-gray-600">Max Temperature</span>
//                   <span className="font-semibold text-red-600">{analytics.statistics.maxTemperature}°C</span>
//                 </div>
                
//                 <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
//                   <span className="text-gray-600">Min Temperature</span>
//                   <span className="font-semibold text-green-600">{analytics.statistics.minTemperature}°C</span>
//                 </div>

//                 <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
//                   <span className="text-gray-600">Total Rainfall</span>
//                   <span className="font-semibold text-purple-600">{analytics.statistics.totalRainfall} mm</span>
//                 </div>

//                 <div className="flex justify-between items-center p-3 bg-indigo-50 rounded-lg">
//                   <span className="text-gray-600">Rain Probability</span>
//                   <span className="font-semibold text-indigo-600">{analytics.statistics.rainProbability}%</span>
//                 </div>
//               </div>
//             </motion.div>
...
//               variants={cardVariants}
//               whileHover="hover"
//               className="bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-2xl shadow-lg p-6"
//             >
//               <h2 className="text-2xl font-bold mb-4 flex items-center">
//                 <span className="mr-2">💡</span>
//                 Irrigation Advice
...
//                   {analytics.irrigationRecommendation.level === 'high' && "🔥"}
//                   {analytics.irrigationRecommendation.level === 'medium-high' && "☀️"}
//                   {analytics.irrigationRecommendation.level === 'medium' && "💨"}
//                   {analytics.irrigationRecommendation.level === 'low' && "💧"}
//                   {analytics.irrigationRecommendation.level === 'very-low' && "🌧️"}
//                   {analytics.irrigationRecommendation.level === 'normal' && "✅"}
//                 </div>
//                 <p className="text-xl font-semibold mb-2">{analytics.irrigationRecommendation.recommendation}</p>
//                 <p className="text-white text-opacity-90 mb-4 text-sm">{analytics.irrigationRecommendation.details}</p>
//                 <div className={`inline-block px-4 py-2 rounded-full ${
//                   analytics.irrigationRecommendation.level === 'high' ? 'bg-red-400' : 
//                   analytics.irrigationRecommendation.level === 'medium-high' ? 'bg-orange-400' : 
//                   analytics.irrigationRecommendation.level === 'medium' ? 'bg-yellow-400' : 
//                   analytics.irrigationRecommendation.level === 'low' ? 'bg-green-400' : 
//                   analytics.irrigationRecommendation.level === 'very-low' ? 'bg-blue-400' : 'bg-gray-400'
//                 }`}>
//                   <span className="font-semibold text-sm">
//                     {analytics.irrigationRecommendation.level.toUpperCase().replace('-', ' ')} LEVEL
//                   </span>
//                 </div>
//               </div>

//               <div className="mt-6 space-y-3">
//                 <div className="flex justify-between items-center p-2 bg-white bg-opacity-20 rounded">
//                   <span className="text-sm">🌡️ Temperature</span>
//                   <span className="font-semibold">{data.current.temperature}°C</span>
//                 </div>
//                 <div className="flex justify-between items-center p-2 bg-white bg-opacity-20 rounded">
...
//                 </div>
//                 <div className="flex justify-between items-center p-2 bg-white bg-opacity-20 rounded">
//                   <span className="text-sm">🌬️ Wind Speed</span>
...
//             className="mt-8"
//           >
//             <h3 className="text-xl font-bold text-green-800 mb-4 text-center">Quick Select Karnataka Cities</h3>
//             <div className="flex flex-wrap justify-center gap-2">
//               {karnatakaCities.slice(0, 8).map((city) => (
//                 <motion.button
//                   key={city.name}
//                   whileHover={{ scale: 1.05 }}
//                   whileTap={{ scale: 0.95 }}
//                   onClick={() => handleCitySelect(city)}
//                   className="bg-white text-green-700 px-4 py-2 rounded-lg border border-green-200 hover:bg-green-50 transition-colors font-medium"
//                 >
//                   {city.name}
//                 </motion.button>
//               ))}
