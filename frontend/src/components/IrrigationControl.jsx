...
const IrrigationControl = ({
  sensorData,        // ✅ Receive real-time sensor data here
  aiEnabled = false,
  onStartNow,
  onSettings,
  loading = false
  ...
  const [nextSchedule, setNextSchedule] = useState("Calculating...");
  const [duration, setDuration] = useState("0 mins");
  const [zone, setZone] = useState("Zone 1");
  const [waterAmount, setWaterAmount] = useState("0 L");
  const [efficiency, setEfficiency] = useState("0%");

  // ✅ Update irrigation info based on live Arduino data
  useEffect(() => {
    if (!sensorData || sensorData.soilMoisture == null) return;

    const { soilMoisture, status } = sensorData;

    if (soilMoisture < 40) {
      setCurrentStatus("Irrigation Needed 💧");
      setNextSchedule("Starting Now");
      setDuration("10 mins");
      setWaterAmount("2.0 L");
      setEfficiency("90%");
    } else if (soilMoisture >= 40 && soilMoisture <= 70) {
      setCurrentStatus("Optimal 🌿");
      setNextSchedule("Next cycle in 2 hrs");
      setDuration("—");
      setWaterAmount("—");
      setEfficiency("95%");
    } else {
      setCurrentStatus("Too Wet ☔");
      setNextSchedule("Postponed");
      setDuration("—");
      setWaterAmount("—");
      setEfficiency("70%");
    }
  ...
  const handleStartNow = () => {
    ...
    else console.log("Manual irrigation triggered");
  };

  const handleSettings = () => {
    if (onSettings) onSettings();
    else console.log("Open irrigation settings");
  };
